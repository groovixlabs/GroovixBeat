...
AudioProcessorGraph::Node::Ptr PluginGraph::addPluginSync (const PluginDescriptionAndPreference& desc, Point<double> pos)
{
    auto dpiDisabler = makeDPIAwarenessDisablerForPlugin (desc.pluginDescription);

    String errorMessage;
    auto instance = formatManager.createPluginInstance (desc.pluginDescription,
                                                        graph.getSampleRate(),
                                                        graph.getBlockSize(),
                                                        errorMessage);
    ...
    if (instance == nullptr)
    {
        DBG("PluginGraph::addPluginSync - Failed to create plugin: " + errorMessage);
        return nullptr;
    }

   #if JUCE_PLUGINHOST_ARA && (JUCE_MAC || JUCE_WINDOWS || JUCE_LINUX)
    if (desc.useARA == PluginDescriptionAndPreference::UseARA::yes
    ...
    if (auto node = graph.addNode (std::move (instance)))
    {
        ...
        node->properties.set ("useARA", desc.useARA == PluginDescriptionAndPreference::UseARA::yes);
        changed();

        DBG("PluginGraph::addPluginSync - Created plugin node: " + String((int)node->nodeID.uid));
        return node;
    }

    DBG("PluginGraph::addPluginSync - Failed to add node to graph");
    return nullptr;
}
...
//==============================================================================
void PluginGraph::sendMidiToPlugins(const juce::MidiMessage& message)
{
    juce::ScopedLock sl(midiLock);
    sequencerMidiBuffer.addEvent(message, 0);
}

void PluginGraph::sendMidiToPlugins(const juce::MidiBuffer& buffer)
{
    juce::ScopedLock sl(midiLock);
    sequencerMidiBuffer.addEvents(buffer, 0, -1, 0);
}
