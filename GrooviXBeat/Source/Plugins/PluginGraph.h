...
public:
    ...
    /** Create a plugin synchronously and add it to the graph. Returns the node or nullptr on failure. */
    AudioProcessorGraph::Node::Ptr addPluginSync (const PluginDescriptionAndPreference&, Point<double>);
    ...
    //==============================================================================
    // MIDI input from sequencer

    void sendMidiToPlugins(const juce::MidiMessage& message);
    void sendMidiToPlugins(const juce::MidiBuffer& buffer);
...
private:
    ...
    // MIDI buffer for sequencer input
