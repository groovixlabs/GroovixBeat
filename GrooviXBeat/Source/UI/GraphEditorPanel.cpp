                                                 ...
                                                 private AsyncUpdater
{
    ...
    {
        // Neumorphic shadow - darker and more pronounced
        shadow.setShadowProperties (DropShadow (Colours::black.withAlpha (0.7f), 8, { 4, 4 }));
    ...
    void mouseDrag (const MouseEvent& e) override
    {
        ...
        if (! e.mods.isPopupMenu())
        {
            ...
            constexpr int snapSize = 50;
            pos.x = ((pos.x + snapSize / 2) / snapSize) * snapSize;
            pos.y = ((pos.y + snapSize / 2) / snapSize) * snapSize;
    ...
    void paint (Graphics& g) override
    {
        auto boxArea = getLocalBounds().reduced (4, pinSize).toFloat();
        ...
        // Gold color #d5a865
        auto boxColour = Colour (0xffd5a865);

        if (isBypassed)
            boxColour = boxColour.withAlpha (0.5f);

        constexpr float cornerRadius = 8.0f;

        // Draw neumorphic light highlight (top-left inner glow)
        g.setColour (Colours::white.withAlpha (0.1f));
        g.drawRoundedRectangle (boxArea.translated (-1.0f, -1.0f), cornerRadius, 2.0f);

        // Fill the main box with rounded corners
        g.setColour (boxColour);
        g.fillRoundedRectangle (boxArea, cornerRadius);

        // Draw subtle border
        g.setColour (boxColour.darker (0.2f));
        g.drawRoundedRectangle (boxArea, cornerRadius, 1.0f);

        // Draw text in dark color for contrast
        g.setColour (Colours::black.withAlpha (0.85f));
        g.setFont (font);
        g.drawFittedText (getName(), boxArea.toNearestInt(), Justification::centred, 2);
    }
...
void GraphEditorPanel::createNewPlugin (const PluginDescriptionAndPreference& desc, Point<int> position)
{
    // Snap to 50 pixel grid
    constexpr int snapSize = 50;
    position.x = ((position.x + snapSize / 2) / snapSize) * snapSize;
    position.y = ((position.y + snapSize / 2) / snapSize) * snapSize;
...
void GraphEditorPanel::showPopupMenu (Point<int> mousePos)
{
    ...
    // Find MainHostWindow - it may not be a parent if GraphEditorPanel is in a separate window
    MainHostWindow* mainWindow = findParentComponentOfClass<MainHostWindow>();

    // If not found in parent hierarchy, search among top-level windows
    if (mainWindow == nullptr)
    {
        for (int i = 0; i < TopLevelWindow::getNumTopLevelWindows(); ++i)
        {
            if (auto* tlw = dynamic_cast<MainHostWindow*> (TopLevelWindow::getTopLevelWindow (i)))
            {
                mainWindow = tlw;
                break;
            }
    ...
    if (mainWindow != nullptr)
    {
        ...
        // Capture mainWindow pointer for the callback
        menu->showMenuAsync ({},
                             ModalCallbackFunction::create ([this, mousePos, mainWindow] (int r)
                                                            {
                                                                if (const auto chosen = mainWindow->getChosenType (r))
...
//==============================================================================
class GraphDocumentComponent::GraphPanelWindow final : public DocumentWindow
{
public:
    GraphPanelWindow (GraphEditorPanel* panel)
        : DocumentWindow ("Plugin Graph",
                          Colour (0xff1a1a1a),
                          DocumentWindow::allButtons),
          graphPanel (panel)
    {
        setContentNonOwned (graphPanel, false);
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (GraphPanelWindow)
};
...
private:
    void paint (Graphics& g) override
    {
        // Use slightly darker shade of main background #1a1a1a
...
void GraphDocumentComponent::init()
{
    ...
    // Create sequencer component (shown by default)
    sequencerComponent.reset(new SequencerComponent(*this, *graph));
    addAndMakeVisible (sequencerComponent.get());

    // Graph panel window (hidden by default, can be toggled via menu)
    graphPanelWindow.reset (new GraphPanelWindow (graphPanel.get()));

    keyboardComp.reset (new MidiKeyboardComponent (keyState, MidiKeyboardComponent::horizontalKeyboard));
    keyboardComp->setVisible (false);  // Hidden by default
    addChildComponent (keyboardComp.get());
...
void GraphDocumentComponent::resized()
{
    ...
    // Only allocate space for keyboard if visible
    if (keyboardComp != nullptr && keyboardComp->isVisible())
        keyboardComp->setBounds (r.removeFromBottom (keysHeight));

    statusBar->setBounds (r.removeFromBottom (statusHeight));

    // Sequencer is the main view
    if (sequencerComponent != nullptr)
        sequencerComponent->setBounds (r);
...
void GraphDocumentComponent::toggleGraphPanel()
{
    if (graphPanelWindow != nullptr)
    {
        bool shouldShow = !graphPanelWindow->isVisible();
        graphPanelWindow->setVisible (shouldShow);

        if (shouldShow)
        {
            graphPanelWindow->toFront (true);
            if (graphPanel != nullptr)
                graphPanel->updateComponents();
        }
    }
...
bool GraphDocumentComponent::isGraphPanelVisible() const
{
    return graphPanelWindow != nullptr && graphPanelWindow->isVisible();
}

void GraphDocumentComponent::toggleMidiKeyboard()
{
    if (keyboardComp != nullptr)
    {
        keyboardComp->setVisible (!keyboardComp->isVisible());
        resized();
    }
}

bool GraphDocumentComponent::isMidiKeyboardVisible() const
{
    return keyboardComp != nullptr && keyboardComp->isVisible();
}

void GraphDocumentComponent::pingSequencer()
{
    if (sequencerComponent != nullptr)
        sequencerComponent->pingSequencer();
}

void GraphDocumentComponent::saveSequencerState()
{
    if (sequencerComponent != nullptr)
        sequencerComponent->saveSequencerState();
}

void GraphDocumentComponent::loadSequencerState()
{
    if (sequencerComponent != nullptr)
        sequencerComponent->loadSequencerState();
}

void GraphDocumentComponent::newProject()
{
    if (sequencerComponent != nullptr)
        sequencerComponent->newProject();
}

void GraphDocumentComponent::openProject()
{
    if (sequencerComponent != nullptr)
        sequencerComponent->openProject();
}
